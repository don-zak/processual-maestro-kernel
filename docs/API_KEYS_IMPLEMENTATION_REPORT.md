...
## KEY-10 — Persistence Hardening / JSON Store Safety

### الهدف

تهدف KEY-10 إلى تقوية سلامة التخزين المحلي لملفات JSON الحساسة قبل الانتقال إلى مراحل النشر السحابي أو Billing.

التركيز في هذه المرحلة كان على حماية ملف إعدادات المستخدم ومفاتيح API من التلف أثناء الكتابة، خصوصًا أن هذا الملف يحتوي على:
...
قبل KEY-10 كانت دالة الحفظ المحلية:

```python
_save_raw(user_id, data)
```

تكتب مباشرة إلى ملف JSON عبر:
...
هذا كان يحمي من التزامن جزئيًا، لكنه لا يمنع خطر تلف الملف إذا توقف السيرفر أو النظام أثناء عملية الكتابة.
...
حتى تعتمد منطق كتابة أكثر أمانًا:

```text
1. تجهيز JSON payload في الذاكرة
2. كتابة المحتوى أولًا في ملف مؤقت .tmp
3. إنشاء نسخة backup من الملف القديم .bak إن كان موجودًا
4. استبدال الملف الأصلي عبر tmp_path.replace(path)
5. تنظيف ملف .tmp عند الفشل
```
...
أصبحت الكتابة تتم بهذا المنطق:

```text
settings_x.json.tmp  -> كتابة مؤقتة
settings_x.json.bak  -> نسخة احتياطية من آخر نسخة سليمة
settings_x.json      -> الملف النهائي بعد الاستبدال
```

وبذلك لم تعد الكتابة تتم مباشرة فوق الملف الأصلي.

---

### الإثبات العملي

بعد تشغيل النظام وتنفيذ عملية إنشاء/حذف مفتاح اختبار، تم إثبات ظهور ملف backup محليًا:

```text
settings_api_key_user.json
settings_api_key_user.json.bak
```

وهذا يثبت أن `_save_raw` أصبح ينشئ نسخة احتياطية عند الكتابة.

---

### حدود KEY-10 الحالية

KEY-10 في هذه المرحلة لا تنقل التخزين إلى PostgreSQL بعد.

هي مرحلة تقوية للتخزين المحلي الحالي، وتشمل:

```text
atomic write
backup before replace
cleanup of temporary file
preservation of existing file_lock
```

أما الانتقال إلى قاعدة بيانات مثل PostgreSQL فيبقى مرحلة لاحقة بعد استكمال مراجعة النشر واحتياجات Cloud Run.
...
```text
KEY-10 Persistence Hardening / JSON Store Safety: PASS
```

أصبحت دالة حفظ إعدادات المستخدم ومفاتيح API أكثر أمانًا، مع تقليل خطر تلف ملفات JSON أثناء عمليات إنشاء المفاتيح أو تعديل الخطط أو الحصص أو إلغاء المفاتيح.

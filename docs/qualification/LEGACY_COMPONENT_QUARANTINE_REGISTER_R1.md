...
| `readiness_report.html` | `QUARANTINED_SOURCE` | Current qualification evidence and governed PR exact-head records | Root historical readiness artifact is retained in Git but never copied into the public runtime image; Docker uses an explicit allowlist and CI rejects a broad `COPY . .` path that could reconnect it | Reconcile historical readiness language against final release truth; confirm no documentation/audit requirement needs the file in-tree before any dedicated deletion review |
...
| `processual_api/billing/maestro_units.py::credits_from_maestro_units` | `DELETE_CANDIDATE` | Canonical Maestro Units metric; no production caller found for this conversion helper | Function remains source-present while deletion review checks external import/API compatibility; CI rejects any new production reference outside its defining module. The separate `credits` metric read/migration alias remains active and is not a deletion candidate | Confirm no external/public import promise; remove helper and its dedicated compatibility test in one isolated change; keep legacy metric normalization until its own migration gate closes |
...
4. Root historical readiness artifacts classified as `QUARANTINED_SOURCE` must remain outside the runtime-image allowlist; broad repository copies that could reconnect them are forbidden.
5. Navigation/page shells linked exclusively to quarantined behavior must not be user-visible.
6. Quarantine must be enforced in tests against rendered application responses and image/build contracts, not only source inspection.
7. Quarantine does not authorize deletion; Git remains the review/evidence location until the deletion gate passes.
...
The four legacy browser CGT/Governor JavaScript files are source-retained in Git, runtime-disconnected, and absent from the public Docker runtime image. `readiness_report.html` is retained only as a historical root artifact and is excluded by the runtime-image allowlist. `subscription_catalog.py`, `client_provider_alias_18.py`, and `POST /settings/llm-provider/test` remain compatibility-only. `credits_from_maestro_units()` is a deletion candidate only and cannot regain a production caller; the legacy `credits` metric read/migration alias remains active. `cgt_governor.py` remains active legacy debt and is explicitly **not** a deletion candidate.

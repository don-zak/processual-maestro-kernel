...
def test_standard_prepaid_top_up_plans_are_non_expiring() -> None:
    ...
    for plan_code in (
        ...
        assert (
            top_up_policies[plan_code].rollover_policy
            is TopUpRolloverPolicy.NON_EXPIRING_USAGE_RIGHT
        )
        assert (
            entitlement_policies[
                plan_code
...
def test_integration_starter_rollover_is_contract_defined() -> None:
    ...
    assert (
        ...
        is TopUpRolloverPolicy.CONTRACT_DEFINED
    )
...
def test_enterprise_rollover_is_contract_defined() -> None:
    ...
    for plan_code in (
        ...
        assert (
            policies[plan_code].rollover_policy
            is TopUpRolloverPolicy.CONTRACT_DEFINED
        )

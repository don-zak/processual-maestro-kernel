...
def _inbox(**overrides: object) -> SimpleNamespace:
    ...
    values = {
        ...
        "total_amount": str(subtotal_cents + 500),
        "payload_digest": "a" * 64,
...
@pytest.mark.asyncio
async def test_paid_order_uses_subtotal_before_tax_and_grants_once() -> None:
    inbox = _inbox()
    order = _order()
    cycle = _cycle()
...
@pytest.mark.asyncio
async def test_variant_mismatch_fails_closed_without_grant() -> None:
    uow = FakeUow(inbox=_inbox(variant_id="different"), order=_order(), cycle=_cycle())
    process = process_lemon_squeezy_top_up_factory(uow_factory=lambda: uow)

    with pytest.raises(LemonSqueezyWebhookError, match="authoritative order binding"):
...
@pytest.mark.asyncio
async def test_missing_checkout_variant_binding_fails_closed() -> None:
    uow = FakeUow(
        inbox=_inbox(),
        order=_order(provider_variant_id=None),
        cycle=_cycle(),
    )
    process = process_lemon_squeezy_top_up_factory(uow_factory=lambda: uow)
...
@pytest.mark.asyncio
async def test_provider_subscription_is_rejected_for_one_time_top_up() -> None:
    uow = FakeUow(
        inbox=_inbox(provider_subscription_id="123"),
        order=_order(),
        cycle=_cycle(),
    )
    process = process_lemon_squeezy_top_up_factory(uow_factory=lambda: uow)

    with pytest.raises(LemonSqueezyWebhookError, match="must not be bound"):
        await process(inbox_id=INBOX_ID, processed_at=NOW)


@pytest.mark.asyncio
async def test_subtotal_mismatch_fails_closed_even_when_total_could_match() -> None:
    uow = FakeUow(
        inbox=_inbox(subtotal_amount="1"),
        order=_order(),
        cycle=_cycle(),
    )
    process = process_lemon_squeezy_top_up_factory(uow_factory=lambda: uow)

    with pytest.raises(LemonSqueezyWebhookError, match="subtotal conflicts"):
        await process(inbox_id=INBOX_ID, processed_at=NOW)

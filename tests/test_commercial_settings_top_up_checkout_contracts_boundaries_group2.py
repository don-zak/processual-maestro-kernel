...
MODULE = Path("processual_api/billing/commercial_settings_top_up_checkout_contracts.py")
...
def test_checkout_contract_has_no_network_or_persistence_dependency() -> None:
    modules: set[str] = set()
...
def test_checkout_contract_executes_no_side_effects() -> None:
    calls: set[str] = set()
    for node in ast.walk(_tree()):

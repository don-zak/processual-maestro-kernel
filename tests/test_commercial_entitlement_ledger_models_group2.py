...
def test_model_table_names_match_schema_contracts() -> None:
    assert CommercialEntitlementLedgerEntry.__tablename__ == (LEDGER_ENTRIES_TABLE)
    assert CommercialEntitlementBalance.__tablename__ == (BALANCES_TABLE)
    assert CommercialEntitlementReservationLock.__tablename__ == (RESERVATION_LOCKS_TABLE)


def test_all_schema_contract_columns_exist_in_models() -> None:
    model_by_table = {model.__tablename__: model for model in COMMERCIAL_ENTITLEMENT_LEDGER_MODELS}

    for table_contract in build_entitlement_schema_contracts():
        model = model_by_table[table_contract.name]

        assert set(model.__table__.columns.keys()) == {column.name for column in table_contract.columns}


def test_ledger_entry_primary_key_is_entry_id() -> None:
    primary_keys = {column.name for column in (CommercialEntitlementLedgerEntry.__table__.primary_key.columns)}

    assert primary_keys == {"entry_id"}


def test_balance_primary_key_is_composite_scope() -> None:
    primary_keys = {column.name for column in (CommercialEntitlementBalance.__table__.primary_key.columns)}
...
def test_reservation_lock_primary_key_is_composite_scope() -> None:
    primary_keys = {column.name for column in (CommercialEntitlementReservationLock.__table__.primary_key.columns)}
...
def test_ledger_scoped_idempotency_constraint_exists() -> None:
    constraints = [
        constraint
        for constraint in (CommercialEntitlementLedgerEntry.__table__.constraints)
...
def test_all_check_constraints_are_explicitly_named() -> None:
    for model in COMMERCIAL_ENTITLEMENT_LEDGER_MODELS:
        ...
        assert checks
        assert all(constraint.name is not None for constraint in checks)
...
def test_indexes_are_explicitly_named() -> None:
    for model in COMMERCIAL_ENTITLEMENT_LEDGER_MODELS:
        ...
        assert indexes
        assert all(isinstance(index, Index) and index.name is not None for index in indexes)
...
def test_model_constraints_match_schema_contract_names() -> None:
    ...
    for contract in build_entitlement_schema_contracts():
        ...
        expected = {constraint.name for constraint in contract.constraints}

        assert expected.issubset(table_constraints(model))
...
def test_model_indexes_match_schema_contract_names() -> None:
    ...
    for contract in build_entitlement_schema_contracts():
        model = model_by_table[contract.name]
        expected = {index.name for index in contract.indexes}
...
def test_ledger_entry_related_foreign_key_is_restrictive() -> None:
    column = CommercialEntitlementLedgerEntry.__table__.columns["related_entry_id"]
    foreign_keys = list(column.foreign_keys)

    assert len(foreign_keys) == 1
    assert foreign_keys[0].ondelete == "RESTRICT"
    assert foreign_keys[0].constraint.name == ("fk_commercial_entitlement_ledger_entries_related_entry")


def test_ledger_entry_is_append_only_by_contract() -> None:
    assert CommercialEntitlementLedgerEntry in (COMMERCIAL_ENTITLEMENT_LEDGER_MODELS)

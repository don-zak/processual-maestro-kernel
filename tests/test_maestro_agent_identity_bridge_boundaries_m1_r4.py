...
MODULE_PATH = Path("processual_api/billing/maestro_agent_identity_bridge.py")

ALLOWED_INTERNAL_IMPORTS = {
    "processual_api.billing.maestro_agent_identity_carrier",
}
...
def test_module_imports_only_carrier_contract() -> None:
...
def test_contract_does_not_import_runtime_types() -> None:
...
def test_contract_defines_no_store_measurement_or_observer() -> None:
...
def test_receipt_default_flags_are_false() -> None:
    source = MODULE_PATH.read_text(encoding="utf-8-sig")

    assert "accepted_for_execution: bool = False" in source
    assert "measurement_emitted: bool = False" in source
    assert "persisted: bool = False" in source

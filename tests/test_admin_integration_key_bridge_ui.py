...
def test_admin_api_key_panel_accepts_bridge_context() -> None:
    js = _read("processual_api/static/js/admin_api_keys.js")

    assert "Integration request bridge" in js
    assert "PMK_ADMIN_INTEGRATION_KEY_BRIDGE" in js
    assert "pmk-admin-integration-key-bridge" in js
    assert "pmk_admin_integration_key_bridge" in js
    ...
    assert "Production connector approval remains separate" in js


def test_admin_html_loads_bridge_capable_scripts() -> None:
    html = _read("processual_api/static/admin.html")

    assert "admin_api_keys.js" in html

...
MODULE = Path("processual_api/billing/commercial_settings_top_up_ui_contracts.py")
...
def test_settings_contract_has_no_frontend_framework_dependency() -> None:
    modules: set[str] = set()
    ...
    forbidden = ("react", "vue", "svelte", "next", "vite", "tailwind")
    for module in modules:
        assert not any(item in module.lower() for item in forbidden)


def test_settings_contract_executes_no_commercial_side_effects() -> None:
    calls: set[str] = set()
    for node in ast.walk(_tree()):

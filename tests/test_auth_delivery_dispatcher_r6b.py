...
def _recovery_claim(*, now, attempt_count=1, user_status="active", state="pending", revoked_at=None, expires_at=None):
    cipher = _cipher()
    outbox_id = uuid.uuid4()
    user_id = uuid.uuid4()
...
def test_unexpected_provider_crash_is_retried_and_batch_continues(caplog):
    ...
    unexpected_records = [record for record in caplog.records if record.getMessage() == "identity_delivery_provider_unexpected_failure"]
...
def test_max_attempt_failure_moves_claim_to_dead_letter():
    ...
    repository = FakeRepository([claim])
    result = asyncio.run(_dispatcher(repository, FakeProvider("provider_timeout"), now=now, max_attempts=3).dispatch_once())
    assert result.dead_lettered == 1
    assert result.retry_scheduled == 0
    assert repository.failed[0]["dead_lettered_at"] == now

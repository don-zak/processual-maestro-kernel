...
@pytest.mark.asyncio
async def test_runtime_wires_all_required_authorities(monkeypatch):
    async def fake_get_redis():
        return FakeRedis()

    monkeypatch.setattr(runtime_module, "get_redis", fake_get_redis)
    monkeypatch.setattr(runtime_module, "get_session_factory", lambda: object())

    runtime = await build_registration_runtime(_config())
...
async def test_runtime_fails_closed_for_missing_or_unsafe_authority(monkeypatch, updates):
    async def fake_get_redis():
        return FakeRedis()
    ...
    with pytest.raises(RegistrationRuntimeUnavailableError):
        await build_registration_runtime(_config(**updates))


@pytest.mark.asyncio
async def test_runtime_fails_closed_when_redis_is_missing(monkeypatch):
    async def missing_redis():
        return None

    monkeypatch.setattr(runtime_module, "get_redis", missing_redis)

    with pytest.raises(RegistrationRuntimeUnavailableError):
        await build_registration_runtime(_config())

...
class PlanFulfillmentSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.seat_based_consumption:
            raise ValueError(
                "Maestro consumption must be quota based, not seat based"
            )
...
def get_plan_fulfillment_spec(
    ...
    try:
        ...
        raise KeyError(
            f"unknown authoritative plan: {normalized or '(blank)'}"
        ) from exc


def monthly_unit_allowance(plan_code: str | None) -> int:
    try:
        return get_plan_fulfillment_spec(
            plan_code

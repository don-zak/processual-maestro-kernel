...
from processual_api.admin_marketplace.subscription_access import resolve_subscription_access
...
from processual_api.billing.direct_checkout_router import router as direct_checkout_router
from processual_api.billing.offer_pricebook import public_offer_pricebook
from processual_api.billing.plan_capability_router import router as plan_capability_router
...
from processual_api.billing.subscription_preparation import build_subscription_preparation
...
def _billing_user_id(current_user: dict) -> str:
    return str(current_user.get("user_id") or current_user.get("sub") or "").strip()


def _require_billing_admin(current_user: dict) -> None:
    role = str(current_user.get("role") or current_user.get("admin_role") or "").strip()
    raw_scopes = current_user.get("scopes") or current_user.get("permissions") or []
    scopes = {str(scope).strip() for scope in ([raw_scopes] if isinstance(raw_scopes, str) else raw_scopes)}
    if role in {"admin", "administrator", "owner_admin", "billing_admin"}:
        return
    if scopes.intersection({"admin:*", "admin:billing:read", "admin:billing:write"}):
        return
    raise HTTPException(status_code=403, detail="Billing statement supervisor access denied.")
...
def _statement_summary(statement: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "additional_package_count": len(statement.get("additional_packages") or []),
        "reconciled": bool(statement["reconciliation"]["reconciled"]),
        "top_ups_reconciled": bool(statement["reconciliation"]["top_ups_reconciled"]),
        "pdf_url": f"/billing/statements/{statement['statement_ref']}/pdf",
    }


async def _issue_statement(*, client_id: str, user_id: str, period: str) -> dict[str, Any]:
    try:
        ...
        raise HTTPException(status_code=503, detail="Billing statement authority is unavailable.") from exc
...
def _load_verified_statement(statement_ref: str) -> dict[str, Any]:
    try:
        ...
        raise HTTPException(status_code=409, detail="Billing statement integrity verification failed.") from exc


@router.post("/checkout", response_model=dict)
async def create_checkout(body: dict, current_user: dict = Depends(get_current_user)) -> dict[str, object]:
    ...
    try:
        ...
        attributes: dict[str, Any] = {
            "store_id": int(store_id), "variant_id": int(variant_id),
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            response = await client.post(
                "https://api.lemonsqueezy.com/v1/checkouts", json=payload,
                headers={"Authorization": f"Bearer {api_key}", "Accept": "application/json", "Content-Type": "application/vnd.api+json"},
            )
    ...
    return {"url": data.get("attributes", {}).get("url", ""), "checkout_id": data.get("id", "")}


@router.get("/portal", response_model=dict)
async def customer_portal(current_user: dict = Depends(get_identity_user)) -> dict[str, object]:
    customer_ref = _identity_customer_ref(current_user)
    try:
    ...
    raise HTTPException(status_code=409, detail="Customer portal is unavailable until provider binding is verified.")


@router.get("/subscription-preparation", response_model=dict)
async def get_subscription_preparation(current_user: dict = Depends(get_identity_user)) -> dict[str, object]:
    try:
    ...
    try:
        ...
        async with session_factory() as session:
            return await build_subscription_preparation(repository=SqlAlchemySubscriptionPreparationRepository(session), user_id=user_id)
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=503, detail="Subscription preparation is unavailable.") from exc


@router.get("/subscription", response_model=dict)
async def get_billing_subscription(current_user: dict = Depends(get_current_user)) -> dict[str, object]:
    customer_ref = _identity_customer_ref(current_user)
    try:
        snapshot = await resolve_subscription_access(customer_ref)
    except Exception as exc:
        raise HTTPException(status_code=503, detail="Billing service is temporarily unavailable.") from exc
    if snapshot is None:
        return {"plan": None, "status": "inactive", "billing_provider": "lemonsqueezy", "has_subscription": False}
    return {
        "subscription_id": str(snapshot.subscription_id), "plan": snapshot.entitlement_profile_ref,
        "status": snapshot.access_stage, "renews_at": snapshot.grace_until,
        "billing_provider": "lemonsqueezy", "has_subscription": True,
    }


@router.get("/statements", response_model=dict)
async def list_customer_billing_statements(current_user: dict = Depends(get_identity_user)) -> dict[str, Any]:
    client_id = _identity_customer_ref(current_user)
    items = [_statement_summary(item) for item in list_statements(_DATA_DIR, client_id=client_id)]
    return {"status": "ready", "client_id": client_id, "statement_count": len(items), "statements": items}


@router.post("/statements/{period}", response_model=dict, status_code=201)
async def issue_customer_billing_statement(period: str, current_user: dict = Depends(get_identity_user)) -> dict[str, Any]:
    client_id = _identity_customer_ref(current_user)
    statement = await _issue_statement(client_id=client_id, user_id=_billing_user_id(current_user) or client_id, period=period)
    return {"status": "issued", "statement": statement}


@router.get("/statements/{statement_ref}", response_model=dict)
async def get_customer_billing_statement(statement_ref: str, current_user: dict = Depends(get_identity_user)) -> dict[str, Any]:
    client_id = _identity_customer_ref(current_user)
    ...
    return {"status": "verified", "statement": statement}


@router.get("/statements/{statement_ref}/pdf")
async def download_customer_billing_statement_pdf(statement_ref: str, current_user: dict = Depends(get_identity_user)) -> Response:
    client_id = _identity_customer_ref(current_user)
    statement = _load_verified_statement(statement_ref)
    if str(statement.get("client_id") or "") != client_id:
        raise HTTPException(status_code=404, detail="Billing statement not found.")
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="{statement_ref}.pdf"', "X-Maestro-Statement-SHA256": statement["statement_sha256"]},
    )


@router.get("/admin/statements", response_model=dict)
async def list_admin_billing_statements(client_id: str | None = None, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    _require_billing_admin(current_user)
    items = [_statement_summary(item) for item in list_statements(_DATA_DIR, client_id=client_id)]
    return {"status": "ready", "statement_count": len(items), "statements": items}


@router.post("/admin/statements/{client_id}/{period}", response_model=dict, status_code=201)
async def issue_admin_billing_statement(client_id: str, period: str, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    _require_billing_admin(current_user)
    try:
        normalized_client_id = str(uuid.UUID(client_id))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Invalid billing client identifier.") from exc
    statement = await _issue_statement(client_id=normalized_client_id, user_id=normalized_client_id, period=period)
    return {"status": "issued", "statement": statement}


@router.get("/admin/statements/{statement_ref}", response_model=dict)
async def get_admin_billing_statement(statement_ref: str, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    _require_billing_admin(current_user)
    return {"status": "verified", "statement": _load_verified_statement(statement_ref)}


@router.get("/admin/statements/{statement_ref}/pdf")
async def download_admin_billing_statement_pdf(statement_ref: str, current_user: dict = Depends(get_current_user)) -> Response:
    _require_billing_admin(current_user)
    statement = _load_verified_statement(statement_ref)
    return Response(
        content=render_statement_pdf(statement),

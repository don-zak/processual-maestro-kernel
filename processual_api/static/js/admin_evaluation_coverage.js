(function () {
  ...
  function renderStatus(status, quality, taskQuality) {
    ...
    target.innerHTML = `
      ...
      <div class="admin-note" style="margin-top:var(--s-2)"><strong>Default quality evidence thresholds:</strong> at least ${escapeHtml(quality?.thresholds?.min_successes_per_endpoint || 3)} successful runs per protected endpoint; maximum failure rate ${escapeHtml(quality?.thresholds?.max_failure_rate ?? 0)}; semantic READ outcomes require at least ${escapeHtml(taskQuality?.min_outcome_passes || 3)} clean expected-result passes. A P95 latency limit is evaluated only when explicitly supplied by release policy.</div>

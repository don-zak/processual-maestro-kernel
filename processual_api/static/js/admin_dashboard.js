(function () {
  ...
  function page(pageId) { return document.getElementById(pageId); }
  ...
  function body(cardId) {
    ...
    return card ? card.querySelector('[data-admin-card-body]') : null;
  }

  function table(headers, rows) {
    if (!rows.length) return '<div class="admin-card-note">No rows returned.</div>';
    return [
  ...
  function bars(items) {
    ...
    return [
      ...
      items.map((item) => {
        ...
        return '<div class="admin-bar-row"><div>' + escapeHtml(item.label) + '</div><div class="admin-bar-track"><div class="admin-bar-fill" style="width:' + width + '%"></div></div><div>' + escapeHtml(value) + '</div></div>';
      }).join(''),
      '</div>',
    ].join('');
  }
  ...
  function endpointTable(results) {
    return table(['Endpoint', 'State', 'Status'], results.map((result) => ({
  ...
  async function refreshHome() {
    ...
    write('admin-card-session', endpointTable([session]) + table(['Field', 'Value'], Object.entries(session.data || {}).slice(0, 8).map(([key, value]) => ({ Field: key, Value: typeof value === 'object' ? JSON.stringify(value) : value }))));
    write('admin-card-health', endpointTable([live, ready]) + bars([{ label: 'Live', value: live.ok ? 1 : 0 }, { label: 'Ready', value: ready.ok ? 1 : 0 }]));
    const providerRows = arrayFrom(providers.data, ['providers', 'items', 'data']).map((provider) => ({ Provider: provider.name || provider.provider || provider.provider_id || 'provider', Status: provider.status || provider.state || (provider.ready ? 'ready' : 'unknown'), Ready: provider.ready ?? provider.configured ?? '' }));
    write('admin-card-providers', endpointTable([providers]) + table(['Provider', 'Status', 'Ready'], providerRows) + bars(providerRows.map((row) => ({ label: row.Provider, value: String(row.Ready) === 'true' || row.Status === 'ready' ? 1 : 0 }))));
  }

  async function refreshApiKeys() {
    const keys = await safeGet('/settings/api-keys');
    const rows = arrayFrom(keys.data, ['keys', 'items', 'data']).map((key) => ({ Name: key.name || key.label || key.key_id || key.id || 'key', Status: key.status || (key.revoked ? 'revoked' : 'active'), Scopes: Array.isArray(key.scopes) ? key.scopes.join(', ') : key.scopes || '', Usage: key.usage_count ?? key.used ?? '', LastUsed: key.last_used_at || '' }));
    const active = rows.filter((row) => String(row.Status).toLowerCase() !== 'revoked').length;
    write('admin-card-api-keys', endpointTable([keys]) + table(['Name', 'Status', 'Scopes', 'Usage', 'LastUsed'], rows) + bars([{ label: 'Active', value: active }, { label: 'Revoked', value: rows.length - active }]));
  }
  ...
  async function refreshClients() {
    ...
    write('admin-card-clients', endpointTable([applications, billingEvents, subscriptions]) + '<div class="admin-card-note">Client supervision needs backend routes for applications, subscriptions, billing state, pilot state, and Bridge to Client Console. Missing endpoints are shown explicitly, not mocked.</div>');
  }
  ...
  async function refreshUsage() {
    ...
    write('admin-card-usage', endpointTable([usageLogs, apiKeys, providers]) + bars([{ label: 'API keys', value: keyCount }, { label: 'Providers', value: providerCount }, { label: 'Usage route', value: usageLogs.ok ? 1 : 0 }]) + '<div class="admin-card-note">System usage remains separate from task execution observability below.</div>');
  }
  ...
  async function refreshExecutionObservability() {
    ...
    if (!result.ok || !result.data) {
      write('admin-card-execution-observability', endpointTable([result]) + '<div class="admin-card-note">Execution observability is unavailable. No synthetic execution metrics are displayed.</div>');
      return;
    }
    ...
    const kpiHtml = '<div class="admin-exec-kpis">' + kpis.map(([label, value]) => '<div class="admin-exec-kpi"><span>' + escapeHtml(label) + '</span><strong>' + escapeHtml(value) + '</strong></div>').join('') + '</div>';
  ...
  async function refreshProgress() {
    ...
    write('admin-card-progress', table(['Checkpoint', 'State'], checkpoints.map((item) => ({ Checkpoint: item.label, State: item.value ? 'wired' : 'pending' }))) + bars(checkpoints));
  }
  ...
  async function refreshSystemHealthDetail() {
    ...
    const ready = await safeGet('/health/ready');
    const providers = await safeGet('/adapters/status');
    write('admin-card-health-detail', endpointTable([live, ready, providers]) + '<div class="admin-card-note">System health is operational state; it is intentionally not used as evidence that tasks executed successfully.</div>');
  }
  ...
  window.PMK_ADMIN_DASHBOARD = { refreshDashboard, safeGet, refreshExecutionObservability };

  function boot() { setTimeout(refreshDashboard, 50); }

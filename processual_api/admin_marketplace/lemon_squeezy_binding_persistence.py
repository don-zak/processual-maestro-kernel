...
class AdminMarketLemonSqueezyCustomerBinding(Base):
    ...
    __table_args__ = (
        UniqueConstraint("customer_ref", name="uq_admin_market_ls_customer_binding_customer"),
        UniqueConstraint(
...
class AdminMarketLemonSqueezyBinding(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("provider_order_id", name="uq_admin_market_ls_binding_provider_order"),
        UniqueConstraint(
    ...
    order_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("admin_market_orders.id", ondelete="RESTRICT"), nullable=False
    )
    offer_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("admin_market_offers.id", ondelete="RESTRICT"), nullable=False
    )
    subscription_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("admin_market_subscriptions.id", ondelete="RESTRICT")
    )
    provider_customer_id: Mapped[str] = mapped_column(String(128), nullable=False)
    ...
    last_provider_effective_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
...
class SqlAlchemyLemonSqueezyBindingRepository:
    ...
    async def get_customer_binding_by_customer_ref(
        self, customer_ref: str, *, for_update: bool = False
    ...
    async def get_customer_binding_by_provider_customer_id(
        ...
        statement = select(AdminMarketLemonSqueezyCustomerBinding).where(
            AdminMarketLemonSqueezyCustomerBinding.provider_customer_id == provider_customer_id
        )
        if for_update:
            statement = statement.with_for_update()
        return await self._session.scalar(statement)
    ...
    async def get_by_provider_order_id(
        self, provider_order_id: str, *, for_update: bool = False
    ) -> AdminMarketLemonSqueezyBinding | None:
        statement = select(AdminMarketLemonSqueezyBinding).where(
            AdminMarketLemonSqueezyBinding.provider_order_id == provider_order_id
        )
        if for_update:
            statement = statement.with_for_update()
        return await self._session.scalar(statement)
    ...
    async def get_by_provider_subscription_id(
        self, provider_subscription_id: str, *, for_update: bool = False
    ) -> AdminMarketLemonSqueezyBinding | None:
        statement = select(AdminMarketLemonSqueezyBinding).where(
            AdminMarketLemonSqueezyBinding.provider_subscription_id == provider_subscription_id
        )
        if for_update:
            statement = statement.with_for_update()
        return await self._session.scalar(statement)

    def add_customer_binding(self, binding: AdminMarketLemonSqueezyCustomerBinding) -> None:

...
async def load_lemon_squeezy_reconciliation_context(
    ...
    if order is None:
        raise LemonSqueezyWebhookError("referenced marketplace order was not found.")
    if order.customer_ref != customer_ref:
        raise LemonSqueezyWebhookError("order customer binding conflicts with webhook.")
    if order.selected_channel != "lemon_squeezy":
        raise LemonSqueezyWebhookError("order is not assigned to Lemon Squeezy.")

    offer = await uow.offers.get_by_id(order.offer_id, for_update=True)
    if offer is None:
        raise LemonSqueezyWebhookError("referenced marketplace offer was not found.")
    if offer.offer_code != offer_ref:
        raise LemonSqueezyWebhookError("offer binding conflicts with webhook.")
    if offer.sales_channel != "lemon_squeezy":
        raise LemonSqueezyWebhookError("offer is not assigned to Lemon Squeezy.")
    if offer.billing_period != order.billing_period:
        raise LemonSqueezyWebhookError("order billing period conflicts with offer.")
    ...
    if binding is None:
        raise LemonSqueezyWebhookError("Lemon Squeezy binding was not found.")
    if binding.customer_ref != customer_ref:
        raise LemonSqueezyWebhookError("provider binding belongs to another customer.")
    if binding.offer_id != offer.id:
        raise LemonSqueezyWebhookError("provider binding belongs to another offer.")
    if binding.provider_customer_id != provider_customer_id:
        raise LemonSqueezyWebhookError("provider customer binding conflicts with webhook.")
    ...
    if provider_customer_owner is None or provider_customer_owner.id != binding.id:
        raise LemonSqueezyWebhookError(
    ...
    return LemonSqueezyReconciliationContext(
        ...
        production_mode=not bool(getattr(inbox, "test_mode", False)),
...
__all__ = ["load_lemon_squeezy_reconciliation_context"]

...
class AdministratorInvitation(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_admin_governance_invitations_email_status", "email_normalized", "status", "expires_at"),
        Index("ix_admin_governance_invitations_inviter_created", "invited_by_user_id", "created_at"),
    )
    ...
    invited_by_user_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="RESTRICT"), nullable=False)
    invite_reason: Mapped[str] = mapped_column(String(500), nullable=False)
    expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    accepted_by_user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="SET NULL"))
    ...
    cancelled_by_user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="SET NULL"))
...
class AdministratorInvitationDeliveryOutbox(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('pending', 'processing', 'delivered', 'failed', 'dead')", name="status_allowed"),
        Index("ix_admin_governance_delivery_status_next", "status", "next_attempt_at"),
    )
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    invitation_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("admin_governance_invitations.id", ondelete="CASCADE"), nullable=False)
...
class AdministratorPermissionGrant(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="CASCADE"), nullable=False)
    permission: Mapped[str] = mapped_column(String(120), nullable=False)
    status: Mapped[str] = mapped_column(String(24), nullable=False, default="active")
    source_invitation_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("admin_governance_invitations.id", ondelete="RESTRICT"), nullable=False)
    granted_by_user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="SET NULL"))
    grant_reason: Mapped[str] = mapped_column(String(500), nullable=False)
    granted_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    revoked_by_user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="SET NULL"))
...
class AdministratorGovernanceAuditEvent(Base):
    ...
    actor_user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="SET NULL"))
    subject_user_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("identity_users.id", ondelete="RESTRICT"), nullable=False)
    invitation_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("admin_governance_invitations.id", ondelete="SET NULL"))

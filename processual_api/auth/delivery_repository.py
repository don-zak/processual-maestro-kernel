        ...
        *,
        ...
        async with self._session_factory() as session:
            async with session.begin():
                statement = (
                    select(AuthDeliveryOutbox, IdentityUser, AuthActionToken)
                    .join(IdentityUser, IdentityUser.id == AuthDeliveryOutbox.user_id)
                    .join(AuthActionToken, AuthActionToken.id == AuthDeliveryOutbox.action_token_id)
                    .where(
                    ...
                    .order_by(AuthDeliveryOutbox.available_at, AuthDeliveryOutbox.created_at)
                    .limit(batch_size)
                    .with_for_update(of=AuthDeliveryOutbox, skip_locked=True)
                )
                rows = (await session.execute(statement)).all()
                claims: list[DeliveryClaim] = []
                for outbox, user, action_token in rows:
                    claim_id = uuid.uuid4()
                    ...
                    claims.append(
                        DeliveryClaim(
                            ...
                            action_token_expires_at=action_token.expires_at,
                            action_token_consumed_at=action_token.consumed_at,
                            action_token_invalidated_at=action_token.invalidated_at,

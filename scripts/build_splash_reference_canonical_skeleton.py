...
A reviewed ownership decision is not enough to make geometry canonical: every
unique-owned fragment must be physically connected to its target pin tree in the
measured skeleton. If attaching a reviewed fragment would leave the target tree
with multiple components, the build fails closed and emits the disconnected
attachment IDs in the candidate manifest instead of silently inventing a bridge.
"""
...
def geodesic_partition(skeleton: np.ndarray, seeds: list[tuple[int, int, int]]) -> np.ndarray:
    h, w = skeleton.shape
...
def reconstruct_region_pixels(skeleton, pins, reconciliation):
    seeds = []
    ...
    masks = {"conflict": (labels < 0).astype(np.uint8), "unassigned": (skeleton & (labels == 0)).astype(np.uint8)}
    reconstructed = {}
    ...
    if missing or extra:
        raise SystemExit(f"Reconstructed reconciliation regions differ from audit: missing={missing}, extra={extra}")
    for region_id, audit_region in audit_regions.items():
        expected = int(audit_region.get("area", len(reconstructed[region_id])))
        if len(reconstructed[region_id]) != expected:
            raise SystemExit(f"Region {region_id} area drift: reconstructed={len(reconstructed[region_id])} reviewed={expected}")
    return reconstructed


def region_pixels(regions, reconstructed, region_id):
    region = regions.get(region_id)
    if region is None:
        raise SystemExit(f"Unknown reconciliation region: {region_id}")
    serialized = region.get("pixels")
    pixels = [[int(x), int(y)] for x, y in serialized] if isinstance(serialized, list) and serialized else reconstructed.get(region_id, [])
    if not pixels:
        raise SystemExit(f"No measured pixels available for reviewed region {region_id}")
    expected = int(region.get("area", len(pixels)))
    if len(pixels) != expected:
        raise SystemExit(f"Reviewed region {region_id} pixel mismatch: pixels={len(pixels)} area={expected}")
    return pixels


def component_count(pixels: set[tuple[int, int]], shape: tuple[int, int]) -> int:
    mask = np.zeros(shape, np.uint8)
    for x, y in pixels:
        mask[y, x] = 1
    count, _ = cv2.connectedComponents(mask, 8)
    return count - 1
...
def main() -> None:
    ...
    for item in unique:
        ...
        before_components = component_count(trees[target], (h, w))
        candidate = set(trees[target]) | {(int(x), int(y)) for x, y in pixels}
        after_components = component_count(candidate, (h, w))
        record = {"region": region_id, "target_tree": target, "stage": item.get("stage"), "pixel_count": len(pixels), "before_component_count": before_components, "after_component_count": after_components}
        attachment_records.append(record)
        if after_components != 1:
            disconnected_attachments.append(record)
        trees[target] = candidate
    ...
    for item in shared:
        ...
        pixels = region_pixels(regions, reconstructed, region_id)
        shared_pixels.update((int(x), int(y)) for x, y in pixels)
        shared_records.append({"region": region_id, "target_trees": targets, "pixel_count": len(pixels), "pixels": pixels})
    ...
    for item in unowned:
        region_id = str(item["region"])
        pixels = region_pixels(regions, reconstructed, region_id)
        unowned_pixels.update((int(x), int(y)) for x, y in pixels)
        unowned_records.append({"region": region_id, "reason": item.get("reason"), "pixel_count": len(pixels), "pixels": pixels})
    ...
    for tree_id in range(1, 126):
        ...
        components = component_count(pixels, (h, w))
        if components != 1:
            disconnected_tree_count += 1
        pin = pins[tree_id - 1]
        tree_records.append({"tree_id": tree_id, "pin_id": pin.get("id"), "side": pin.get("side"), "pin": [int(pin["x"]), int(pin["y"])], "seed": [int(pin["seed_x"]), int(pin["seed_y"])], "pixel_count": len(pixels), "component_count": components, "pixels": [[x, y] for x, y in sorted(pixels, key=lambda p: (p[1], p[0]))]})
    ...
    promotion_blocked = bool(disconnected_attachments or disconnected_tree_count)
    manifest = {"meta": {"stage": "CANONICAL_SKELETON_CANDIDATE", "source_of_truth": "approved pivot reference image", "reference_size": list(REFERENCE_SIZE), "canonical_candidate": True, "canonical_promoted": False, "splash_reconstruction_allowed": False, "route_tree_count": 125, "disconnected_route_tree_count": disconnected_tree_count, "disconnected_unique_attachment_count": len(disconnected_attachments), "unique_attachment_count": len(attachment_records), "shared_geometry_region_count": len(shared_records), "preserved_unowned_region_count": len(unowned_records), "reconstructed_review_region_count": len(reconstructed), "owned_pixel_count": len(owned_union), "shared_pixel_count": len(shared_pixels), "preserved_unowned_pixel_count": len(unowned_pixels), "canonical_union_pixel_count": len(canonical_pixels), "owned_shared_overlap_pixel_count": len(overlap_owned_shared), "ownership_ambiguity_count": 0, "promotion_blocked": promotion_blocked, "promotion_gate": "zero disconnected unique attachments + zero disconnected route trees + pixel-accounting + visual-overlay audit"}, "route_trees": tree_records, "unique_attachments": attachment_records, "disconnected_unique_attachments": disconnected_attachments, "shared_geometry": shared_records, "preserved_unowned_geometry": unowned_records}
    cv2.imwrite(str(args.out / "canonical_skeleton_candidate.png"), output)
    (args.out / "canonical_route_manifest_candidate.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print(json.dumps(manifest["meta"], indent=2))
    if promotion_blocked:
        raise SystemExit("Canonical promotion blocked: reviewed unique-owner fragments are not physically connected to their target pin trees")

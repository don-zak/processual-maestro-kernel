...
try {
    ...
    while ((Get-Date) -lt $deadline) {
        if ($server.HasExited) {
            throw "Local review server exited before becoming ready."
        }
        try {
    ...
    if (-not $ready) {
        throw "Local review server did not become ready within $ReadyTimeoutSeconds seconds."
    }
    ...
    $publicPages.Add([pscustomobject]@{ Name = "Access request"; Path = "/apply" })
    $publicPages.Add([pscustomobject]@{ Name = "Registration"; Path = "/register" })
    $publicPages.Add([pscustomobject]@{ Name = "Verification"; Path = "/verify" })
    ...
    foreach ($plan in @($catalog.plans)) {
        if ($null -eq $plan.plan_id -or [string]::IsNullOrWhiteSpace([string]$plan.plan_id)) {
            continue
        }
        $encodedPlan = [System.Uri]::EscapeDataString([string]$plan.plan_id)
        $publicPages.Add([pscustomobject]@{
            Name = "Offer: $($plan.display_name)"
            Path = "/offer/$encodedPlan"
        })
    }

    $reviewPages = [System.Collections.Generic.List[object]]::new()
    foreach ($item in $publicPages) {
        $reviewPages.Add($item)
    }
    ...
    foreach ($item in $reviewPages) {
        ...
        try {
            ...
            if ($ok) {
                Write-Host "[OK]   $($item.Name) -> HTTP $status"
            }
            else {
                Write-Host "[FAIL] $($item.Name) -> HTTP $status" -ForegroundColor Red
            }
        }
        catch {
            $results.Add([pscustomobject]@{
    ...
    if ($failed.Count -gt 0) {
        throw "Full local web review found $($failed.Count) HTTP page failure(s). Browser launch was stopped."
    }
    ...
    if (-not $NoBrowser) {
        ...
        foreach ($item in $publicPages) {
            Start-Process "$base$($item.Path)"
            Start-Sleep -Milliseconds 180
        }
        if ($IncludeAuthenticatedSurfaces) {
            Start-Process "$base/console/"
            Start-Sleep -Milliseconds 180
    ...
    Write-Host "4. Follow registration or assessment CTAs and verify downstream forms, validation, terms, and truth language."
    Write-Host "5. Sign in normally and review Console/Admin only after the public journey is complete."
    Write-Host "6. Review MFA visually during a real challenge and confirm no card compression or hidden-control regression."
    Write-Host "7. Record every defect against source HEAD $head before any Real Staging decision."
    Write-Host ""

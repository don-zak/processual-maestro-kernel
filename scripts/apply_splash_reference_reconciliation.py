...
than inventing pixels. KEEP preserves verified real geometry without forcing
ownership. PENDING decisions are never modified. Multiple decision files may be
combined so independent manual closeouts can remain auditable.
"""
...
def load_decisions(paths: list[Path]) -> dict[str, dict[str, object]]:
    ...
    for path in paths:
        ...
        for item in payload.get("decisions", []):
            ...
            if prior is not None and prior != item:
                raise SystemExit(f"Conflicting decisions for {region}: {path}")
            merged[region] = item
    return merged
...
def main() -> None:
    ...
    if unknown:
        raise SystemExit(f"Decision files reference unknown regions: {unknown}")
    ...
    result = {
        "meta": {
            ...
            "kept_verified_geometry_pixels": kept_pixels,
            "pending_decision_pixels": pending_pixels,
            "decision_counts": decision_counts,
            "decision_file_count": len(args.decisions),
            "remaining_unresolved_region_count": len(unresolved_regions),
            "ownership_ambiguous_kept_region_count": len(kept_regions),
            "completion_gate": "zero-pending-then-resolve-kept-geometry-ownership-before-canonical-promotion",
        },
        "ownership": ownership,
        "kept_geometry_regions": kept_regions,
